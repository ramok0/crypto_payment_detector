use crypto_payment_detector::{
    BasicAuth, Chain, ChainDetector, DetectorConfig, PaymentDetector, RetryConfig,
};
use std::sync::Arc;
...
async fn main() {
    ...
    let chains: Vec<Chain> = match chain_str.to_lowercase().as_str() {
        "both" | "all" => vec![Chain::Bitcoin, Chain::Litecoin],
        other => vec![other.parse().expect("Invalid CHAIN value (expected: bitcoin, litecoin, btc, ltc, both)")],
    };
    ...
    for chain in &chains {
        ...
        let detector = Arc::new(
            ChainDetector::new(config).expect(&format!("Failed to create {} detector", chain.ticker())),
        );
    ...
    if handles.is_empty() {
        eprintln!("No chains configured. Set BTC_XPUB and/or LTC_XPUB.");
        std::process::exit(1);
    }
